...
use crate::{lut_sampler::{TripleVector, online::open_rss_many}, share::{gf_template::{GFTTrait, Share, ShareType}, gf2p64::{GF2p64, GF2p64Subfield}, helper_types::AllowedTypes}, util::mul_triple_vec::DotProdRecorder};
...
impl<T: GFTTrait, IndexType: AllowedTypes, const SIZE1: usize, const SIZE2: usize> MatrixOhv<T, IndexType, SIZE1, SIZE2>{
    ...
    pub fn collapse_cols<const SIZE2_RED: usize>(&self, matrices: &[[T::Wrapper; SIZE2_RED];2], rec: &mut TripleVector) -> T::Embedded {
        ...
        match rec{
            TripleVector::MAL(r) =>{
                let len = val_i.len();
                let flat_i: Vec<GF2p64>  = <T::Embedded as NetSerializable>::from_byte_vec(val_i, len / new_len).iter().map(|v| v.embed()).collect();
                let flat_ii: Vec<GF2p64> = <T::Embedded as NetSerializable>::from_byte_vec(val_ii, len / new_len).iter().map(|v| v.embed()).collect();
